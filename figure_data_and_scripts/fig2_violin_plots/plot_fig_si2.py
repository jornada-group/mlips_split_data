...
def get_residuals(property_name: str, energy_prop: bool):
    ...
    data = (np.array(residualall)).T[:, np.array([0, 4, 1, 3, 2, 5])]
    rmse_data = (np.array(rmseall))[np.array([0, 4, 1, 3, 2, 5])]
...
def get_residuals_inter(property_name: str, energy_prop: bool):
    ...
    data = (np.array(residualall)).T[:, np.array([0, 5, 3, 1, 4, 2])]
    rmse_data = (np.array(rmseall))[np.array([0, 5, 3, 1, 4, 2])]
    return data, rmse_data
...
def run_plot(property_name, energy_prop, inter, ax, with_gradient=False):
    if not inter:
        colorall = colorintra
        hatchesall = [None, None, "++", "++", "xx", None]
        data, rmse_data = get_residuals(property_name, energy_prop)
    else:
        colorall = colorinter
        hatchesall = [None, None, "xx", "++", "xx", None]
    ...
    extent = [0.5, 7, desired_value * 1e-3, desired_value]

    nplt = 6
    if with_gradient:
        ax.imshow(gradient, aspect="auto", extent=extent)
    ax.hlines(desired_value, 0.5, 7, color=col, linestyle="-", linewidth=2, alpha=0.7)
...
if __name__ == "__main__":
    ...
    fig.align_labels()
    fig.savefig(SCRIPT_DIR / "fig_si_2.pdf")
    fig.savefig(SCRIPT_DIR / "fig_si_2.png")
