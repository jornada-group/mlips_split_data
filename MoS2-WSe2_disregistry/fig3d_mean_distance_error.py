...
if __name__ == "__main__":
    ...
    reference_file = 'giant_struc_2.xyz'
    ...
    structures = {
        'ML relaxed': read("relax_new_2mr6_cfg3_r6_N6.xyz", index=-1, format='extxyz'),
        'DFT relaxed': read("relax_new_2mr6_r6_N10.xyz", index=-1, format='extxyz'),
    }
    ...
    structure_names = ['ML relaxed', 'DFT relaxed']
    ...
    unrelaxed_positions = structures['ML relaxed'].positions[atom_types == 0, :2]
    unrelaxed_positions_padded = pad_periodic_image(unrelaxed_positions, structures['ML relaxed'].cell[:2, :2]).reshape(-1, 2)
    ...
    for i, (name, distance) in enumerate(zip(structure_names, distances)):
        print(f"Distance between ML relaxed and {name}: {distance:.6f} Å")
    print("\n=== Analysis Complete ===\n")